...
技术栈：**Vue3 + Vite + TypeScript**（前端） · **Node.js + Express 风格零依赖服务**（后端，内置 HTTP 服务） · **SSE 流式** · RAG（Embedding 向量检索 + Prompt 拼接） · **sqlite-vec 向量库**（嵌入式 SQLite KNN，可选回退 JSON 零依赖）。
...
| 环节 | 实现 |
|---|---|
| 知识入库 | `server/src/ingest.js`：读 `server/data/source/*.md` → 按段落切片（500-800字、重叠100）→ 向量化 → 写向量库（增量：按文档哈希只重算变化文档） |
| 向量检索 | `server/src/vector-store.js`（门面）→ `store-sqlite.js`：sqlite-vec `vec0` 虚拟表余弦 KNN TopK（默认5条，阈值0.1）；未装依赖自动回退 `store-json.js` |
...
> 向量库说明：默认使用 **sqlite-vec**（`server/data/vectors.db`，嵌入式、零外部服务）。未执行 `npm install` 时会自动回退到 JSON 存储（`VECTOR_STORE=json` 可强制 JSON）。
...
├── README.md
├── server/               # 后端（原生 node:http；向量库可选 sqlite-vec）
│   ├── package.json      # 依赖：better-sqlite3 + sqlite-vec
│   ├── src/
│   │   ├── config.js     # .env 加载 + 配置
│   │   ├── embed.js      # 向量化（SiliconFlow/DashScope/OpenAI + MOCK）
│   │   ├── llm.js        # LLM 流式调用（DeepSeek/通义/OpenAI + MOCK）
│   │   ├── vector-store.js  # 存储层门面：VECTOR_STORE 分发 + 自动回退
│   │   ├── store-sqlite.js # sqlite-vec：vec0 虚拟表余弦 KNN + 元数据表
│   │   ├── store-json.js   # JSON 存储（零依赖回退方案）
│   │   ├── rag.js        # 检索 → 拼 Prompt → 生成
│   │   ├── ingest.js     # 知识入库脚本（增量）
│   │   └── index.js      # HTTP 服务（SSE）
│   └── data/
│       ├── source/       # 知识库原始文档（.md/.txt）
│       ├── vectors.db    # sqlite-vec 向量库（已 gitignore）
│       └── vectors.json  # JSON 回退存储（已 gitignore）
└── web/                  # 前端 Vue3 + Vite + TS
    ├── package.json
...
- **知识库大了怎么入库？** —— 增量入库：按文档哈希只重算变化文档（本仓库已实现）；向量存储已升级 **sqlite-vec**（`vec0` 虚拟表余弦 KNN，支持万级切片），超大知识库可再横向换 Chroma/Qdrant 等专用向量库。
- **为什么用 sqlite-vec 而不是 Chroma？** —— 嵌入式单文件、零外部服务（不用起 Chroma server/Docker）、clone 即跑；SQLite 事务保证向量与元数据一致；且是"存储层抽象 + 平滑替换"的活例子：`VECTOR_STORE` 一行环境变量切换 sqlite/json。
- **成本怎么控？** —— 只对检索命中的片段进 Prompt、历史裁剪
...
- [x] 向量库换 **sqlite-vec**（vec0 余弦 KNN；`VECTOR_STORE` 可回退 JSON；实测增量复用 21 条/0 新增，检索结果与 JSON 版一致）
